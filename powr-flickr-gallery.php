<?php
    /**
     * @package POWr Flickr Gallery
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Flickr Gallery
    Plugin URI: https://www.powr.io/plugins/flickr-gallery
    Description: Get more Flickr followers with a custom gallery  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Flickr Gallery widget/////////////////////////////////
    class Powr_Flickr_Gallery extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_flickr_gallery',
                             __( 'POWr Flickr Gallery' ),
                             array( 'description' => __( 'Flickr Gallery by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-flickr-gallery' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Flickr Gallery.
        </p>
        <p>
    ...
    function register_powr_flickr_gallery() {
      register_widget( 'Powr_Flickr_Gallery' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_flickr_gallery' );
    //Create short codes for adding plugins anywhere:
    function powr_flickr_gallery_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-flickr-gallery' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-flickr-gallery' label='$label'></div>";
      }else{
      	"<div class='powr-flickr-gallery'></div>";
      }
    }
    add_shortcode( 'powr-flickr-gallery', 'powr_flickr_gallery_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_flickr_gallery_menu' );
    function powr_flickr_gallery_menu() {
      add_menu_page( 'POWr Flickr Gallery', 'POWr Flickr Gallery', 'manage_options', 'powr-flickr-gallery-settings', 'powr_flickr_gallery_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_flickr_gallery_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/flickr-gallery';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/flickr-gallery';
      }
    ...
    register_activation_hook(__FILE__, 'powr_flickr_gallery_plugin_activate');
      add_action('admin_init', 'powr_flickr_gallery_plugin_redirect');

      function powr_flickr_gallery_plugin_activate() {
      add_option('powr_flickr_gallery_plugin_do_activation_redirect', true);
      }
      ...
      function powr_flickr_gallery_plugin_redirect() {
      if (get_option('powr_flickr_gallery_plugin_do_activation_redirect', false)) {
          delete_option('powr_flickr_gallery_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-flickr-gallery-settings&' );
          }
       }
      }
